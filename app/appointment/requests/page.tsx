...
export default function AppointmentRequestsPage() {
  ...
  const generateICSFile = async (appointment: AppointmentRequest) => {
    const { studentName, faculty, date, time, duration, reason } = appointment;
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-[#d9601f] to-[#393637] pt-16 pb-10 px-4 md:px-12 grid grid-cols-5 gap-6">
      {/* Increased padding-top from pt-10 to pt-24 */}
      
      {/* Appointment Requests Section */}
      <div className="col-span-1 bg-white/30 backdrop-blur-lg rounded-lg p-6">
        ...
        {loading ? (
          <div className="flex justify-center items-center h-64">
            ...
            <div className="relative w-16 h-16">
              ...
              <div className="absolute inset-0 border-t-4 border-yellow-400 rounded-full animate-spin delay-150"></div>
              <div className="absolute inset-0 border-t-4 border-green-400 rounded-full animate-spin delay-300"></div>
            </div>
          </div>
        ) : (
          <div className="space-y-4">
            {requests.map((req) => (
              <div
                key={req.id}
                className="group bg-white rounded-xl shadow-lg p-4 transition-all duration-300 ease-in-out transform hover:h-auto hover:max-h-[500px] overflow-hidden"
              >
                ...
                <div className="opacity-0 max-h-0 group-hover:max-h-[300px] group-hover:opacity-100 transition-all duration-300 ease-in-out mt-3">
                  ...
                  {req.status === "approved" && req.icsContent && (
                    <button
                      ...
                      className="px-3 py-1 mt-3 text-sm bg-blue-500 text-white rounded hover:bg-blue-600"
                    >
      ...
      {/* Calendar Section */}
      <div className="col-span-4 bg-white/30 backdrop-blur-lg rounded-lg p-6">
        <h2 className="text-xl font-semibold text-white mb-4">Your Calendar</h2>
        <div className="bg-white shadow-lg rounded-xl p-5">
          ...
          <div className="calendar grid grid-cols-7 gap-1">
            ...
            {[...Array(30)].map((_, idx) => (
              <div
                key={idx}
                className="h-16 flex items-center justify-center text-sm font-medium text-gray-800 bg-gray-100 rounded-md"
              >
                {idx + 1}
              </div>
            ))}
